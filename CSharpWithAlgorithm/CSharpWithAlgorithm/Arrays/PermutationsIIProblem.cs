...
public static class PermutationsIIProblem
{
    private static IList<IList<int>> BruteForceSolve(int[] nums)
    {
        ...
        void backtrack(IList<int> candidate, IList<int> source)
        {
            ...
            for (int i = 0; i < numSource; ++i)
            {
                ...
                backtrack(candidate, source);
                source.Insert(i, nextValue);
                candidate.RemoveAt(candidate.Count);
            }
        ...
        return candidateTable.Values.ToList();
    }
    public static IList<IList<int>> PermuteUnique(int[] nums)
    {
        ...
        long numPermutationOf(int[] seq)
            => factorial(seq.Length) / seq.GroupBy(c => c).Aggregate((long)1, (nextResult, pair) => nextResult * factorial(pair.Count()));
        ...
        var numOfLoop = numPermutationOf(nums);

        IList<int> curSeq = nums.ToList();
